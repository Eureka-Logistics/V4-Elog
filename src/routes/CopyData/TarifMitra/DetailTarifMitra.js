    ...
    const fetchSelectMitra = async () => {
      ...
      const EditTarif = async () => {
        try {
          ...
          const response = await axios.post(`${Baseurl}tarif/update-tarifMitra`, data, {
            headers: {
              "Content-Type": "application/json",
              Authorization: localStorage.getItem("token"),
            },
          ...
          if (response.status === 200) {
            ...
            setTimeout(() => {
              window.location.href = "/tarifmitra"; // Replace with the actual path to the "tarif_eureka" page
