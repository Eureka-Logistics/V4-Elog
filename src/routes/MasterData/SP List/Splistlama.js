...
function SPListlama() {
  ...
  const ApiDataAwal = async (page = 1, pageSize = 10) => {
    try {
      const data = await axios.get(`${Baseurl}sp/get-SP-all?limit=${pageSize}&page=${page}&keyword=${search}&statusSP=&customerId=${CustumerValue}&cabang=${CariCabangValue}&sales=${CariSalesValue}&buId=${CariBu}`,
        {
          headers: {
