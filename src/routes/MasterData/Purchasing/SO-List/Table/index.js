...
function SOListTable() {
  ...
  const getlist = async (page = 1) => {
    try {
      const data = await httpClient(
        `${Baseurl}sm/get-list-po?limit=10&page=${page}`,
        { method: "GET" }
      );
      const result = data;
  ...
  const getDetaillist = async (idMpo) => {
    try {
      const data = await httpClient(
        `${Baseurl}sm/get-list-po-id?id_mpo=${idMpo}`,
        { method: "GET" }
      );
      const result = data;
  ...
  return (
    <div>
      <Row>
        <Col sm={2}>
          <Button onClick={createPO} size="" type="primary">
            Buat PO
          </Button>
        </Col>
      </Row>
      ...
      />
      <ModalSOList
        CreatePoModal={CreatePoModal}
        setCreatePoModal={setCreatePoModal}
        createPO={createPO}
        getlist={getlist}
