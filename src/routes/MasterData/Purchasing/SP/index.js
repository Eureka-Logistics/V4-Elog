...
function ListPageSpPurchasing() {
  ...
  const datasp = async (e = 1) => {
    try {
      ...
      const data = await axios.get(
        `${Baseurl}sp/get-list-purch?limit=${paggination.limit}&page=${paggination.currentPage}&is_multi=${MultiSelect}&keyword=${searchSO}`,
        {
          headers: {
      ...
      setasw(response);
      setloading(false);
      setDataRequest(data.data.data.totalData);
      // console.log("data request", data.data.data.totalData);
    } catch (error) {}
  };
  ...
  function selectmulti() {
    if (MultiSelect === 1) {
      return <>Multi Drop</>;
    } else if (MultiSelect === 0) return <>Tidak Multi Drop</>;
  }
        ...
        <div>
          <Row className="mb-3">
            ...
            <Col sm={2}>
              ...
              <Tag
                style={{ backgroundColor: dataRequest === 0 ? "#87d068" : "#f50", color: 'white' }}
              >Request Order in here : {dataRequest}</Tag>
            </Col>
          </Row>
        </div>
        {loading ? (
